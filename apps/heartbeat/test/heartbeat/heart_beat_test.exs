defmodule Heartbeat.HeartBeatTest do
  ...
  test "initialised with empty counts" do
    assert {:ok, %{no_net_count: 0, lan_only_count: 0}} = HeartBeat.init(:_)
  end
  ...
  test "resets counters when the VintagetNetWizard hotspot is up" do
    ...
    assert {:reply, :ok, %{no_net_count: 0, lan_only_count: 0}} =
             HeartBeat.handle_call(:check, {}, %HeartBeat{no_net_count: 2, lan_only_count: 2})
  end
  ...
  test "increments counters when lan, and ip4 is not the hotspot and not on internet" do
    ...
    assert {:reply, :ok, %{no_net_count: 5, lan_only_count: 3}} =
             HeartBeat.handle_call(:check, {}, %HeartBeat{no_net_count: 4, lan_only_count: 2})
  end

  test "resets lan count, increments no_net_count, and kicks VintageNet on 8th lan_only" do
    stub_prop(:lan)

    assert {:reply, :ok, %{no_net_count: 10, lan_only_count: 0}} =
             HeartBeat.handle_call(:check, {}, %HeartBeat{no_net_count: 9, lan_only_count: 7})
  ...
  test "is not ok after 28 no_nets" do
    assert {:reply, :down, _} =
             HeartBeat.handle_call(:check, {}, %HeartBeat{no_net_count: 27, lan_only_count: 2})
  end
